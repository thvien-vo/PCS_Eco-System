  ...
  if (inProgressCards.length === 0) {
    return (
      <div className="flex items-center justify-center gap-2 rounded-2xl border border-dashed border-border bg-card/50 px-4 py-5 text-sm text-muted-foreground">
        ...
        <span>{tm.inProgress.emptyState}</span>
      </div>
    );
  }
  ...
  return (
    ...
    >
      {inProgressCards.map((card) => {
        const displayName = tm.leaderboard.challengeNames[card.name] || card.name;
        return (
          <div
            key={card.id}
          ...
          >
            ...
            <div className="bg-card p-2">
              <p className="line-clamp-1 text-[10px] font-semibold text-foreground">{displayName}</p>
